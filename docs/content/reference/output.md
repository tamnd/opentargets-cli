...
describes what users get. Pick a format with `-o`, or let opentargets choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
opentargets <command> -o table   # aligned columns for reading
opentargets <command> -o jsonl   # one JSON object per line, for piping
opentargets <command> -o json    # a single JSON array
opentargets <command> -o csv     # spreadsheet friendly
opentargets <command> -o tsv     # tab-separated
opentargets <command> -o url     # just the URL column
opentargets <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
opentargets <command> --fields id,title,url
```
...
```bash
opentargets <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
opentargets <command>            # a table, because this is a terminal
opentargets <command> | wc -l    # JSONL, because this is a pipe
```
